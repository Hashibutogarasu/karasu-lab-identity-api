...
describe('LogsService', () => {
  ...
  it('creates a log requiring a valid stone and ownership', async () => {
    ...
    logsRepo.create.mockResolvedValue({
      id: 'l1',
      stoneId: 's1',
      userId: 'u1',
      amount: 50,
    ...
    await expect(service.createLog('s1', 'u1', dto)).rejects.toThrow(
      ErrorCodes.STONEMANAGE.FORBIDDEN,
    );

    stonesRepo.getById.mockResolvedValue(null);
    await expect(service.createLog('invalid', 'u1', dto)).rejects.toThrow(
  ...
  it('gets a log only if the user owns the stone and log belongs to it', async () => {
    ...
    await expect(service.getLog('s1', 'l2', 'u1')).rejects.toThrow(
      ErrorCodes.STONEMANAGE.LOG_NOT_FOUND,
    );
    ...
    await expect(service.getLog('s1', 'l1', 'u1')).rejects.toThrow(
      ErrorCodes.STONEMANAGE.LOG_NOT_FOUND,
    );
  });
  ...
  it('updates a log only if owned', async () => {
    ...
    await expect(
      service.updateLog('s1', 'l1', 'u2', { amount: 100 }),
  ...
  it('sets previousAmount to undefined on first log creation', async () => {
    ...
    logsRepo.findByStoneId.mockResolvedValue([]);
    logsRepo.create.mockImplementation(
  ...
  it('does not modify existing amount when updating previousAmount/nextAmount', async () => {
    ...
    logsRepo.update.mockImplementation(
      (_id: string, data: Record<string, unknown>) =>
        Promise.resolve({
          id: _id,
          stoneId: 's1',
