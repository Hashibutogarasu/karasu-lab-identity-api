...
import { GameModel, StoneModel, StoneLogModel } from '../models/stonemanage.model.js';
import { CreateGameInput, UpdateGameInput, CreateStoneInput, UpdateStoneInput, CreateLogInput } from '../models/stonemanage.input.js';
...
import { RequireOwnership, OwnershipGuard } from '../../../shared/auth/ownership.guard.js';
...
export class GamesResolver {
  ...
  async games(@GqlUser() user: User): Promise<Game[]> {
    const snapshot = await this.firebase.db.collection('games').where('userId', '==', user.id).get();
  ...
  @Mutation(() => GameModel)
  async createGame(@GqlUser() user: User, @Args('input', { type: () => CreateGameInput }) input: CreateGameInput): Promise<Game> {
    const id = cuid();
  ...
  @UseGuards(OwnershipGuard)
  @RequireOwnership('games')
  async updateGame(@Args('id', { type: () => ID }) id: string, @Args('input', { type: () => UpdateGameInput }) input: UpdateGameInput): Promise<Game> {
    const now = new Date().toISOString();
  ...
  @UseGuards(OwnershipGuard)
  @RequireOwnership('games')
  async deleteGame(@Args('id', { type: () => ID }) id: string): Promise<boolean> {
...
export class StonesResolver {
  ...
  @Query(() => [StoneModel])
  async stones(@GqlUser() user: User, @Args('gameId', { type: () => ID, nullable: true }) gameId?: string): Promise<Stone[]> {
    let query = this.firebase.db.collection('stones').where('userId', '==', user.id);
  ...
  @Mutation(() => StoneModel)
  async createStone(@GqlUser() user: User, @Args('input', { type: () => CreateStoneInput }) input: CreateStoneInput): Promise<Stone> {
    const gameDoc = await this.firebase.db.collection('games').doc(input.gameId).get();
  ...
  async updateStone(@Args('id', { type: () => ID }) id: string, @Args('input', { type: () => UpdateStoneInput }) input: UpdateStoneInput): Promise<Stone> {
    const now = new Date().toISOString();
    const data = { ...input, updatedAt: now };
  ...
  @UseGuards(OwnershipGuard)
  @RequireOwnership('stones')
  async deleteStone(@Args('id', { type: () => ID }) id: string): Promise<boolean> {
...
export class StoneLogsResolver {
  ...
  @Query(() => [StoneLogModel])
  async stoneLogs(@GqlUser() user: User, @Args('stoneId', { type: () => ID }) stoneId: string): Promise<StoneLog[]> {
    const stoneDoc = await this.firebase.db.collection('stones').doc(stoneId).get();
    ...
    const snapshot = await this.firebase.db.collection('logs').where('stoneId', '==', stoneId).orderBy('createdAt', 'desc').get();
    return snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as StoneLog));
  }

  @Mutation(() => StoneLogModel)
  async createStoneLog(@GqlUser() user: User, @Args('input', { type: () => CreateLogInput }) input: CreateLogInput): Promise<StoneLog> {
    ...
    const logData = { ...input, userId: user.id, previousAmount, nextAmount, createdAt: now, updatedAt: now };
