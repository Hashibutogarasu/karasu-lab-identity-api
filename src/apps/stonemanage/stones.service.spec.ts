...
describe('StonesService', () => {
  ...
  it('creates a stone requiring a valid game ID', async () => {
    ...
    stonesRepo.create.mockResolvedValue({
      id: 's1',
      gameId: 'g1',
      userId: 'u1',
    });
  ...
  it('gets a stone only if the user owns it and it matches game ID', async () => {
    ...
    await expect(service.getStone('g1', 's1', 'u2')).rejects.toThrow(
      ErrorCodes.STONEMANAGE.FORBIDDEN,
    );
    await expect(service.getStone('g2', 's1', 'u1')).rejects.toThrow(
      ErrorCodes.STONEMANAGE.STONE_NOT_FOUND,
    );
  });
  ...
  it('updates a stone only if owned', async () => {
    ...
    stonesRepo.update.mockResolvedValue({
      id: 's1',
      gameId: 'g1',
      userId: 'u1',
      amount: 50,
    ...
    await expect(
      service.updateStone('g1', 's1', 'u2', { amount: 50 }),
  ...
  it('deletes stones in a game only if the user owns the game', async () => {
    ...
    await expect(service.deleteStonesByGame('g1', 'u2')).rejects.toThrow(
      ErrorCodes.STONEMANAGE.FORBIDDEN,
    );
  });
  ...
  it('creates a stone with imageKey and resolves image URL', async () => {
    ...
    stonesRepo.create.mockResolvedValue({
      ...
      name: 'Gems',
      amount: 100,
      imageKey: 'stonemanage/images/u1/img2',
    });
    const result = await service.createStone('g1', 'u1', {
      name: 'Gems',
      amount: 100,
      imageKey: 'stonemanage/images/u1/img2',
    });
