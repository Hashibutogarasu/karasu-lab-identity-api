...
export class BlogService
  extends AbstractRepository<BlogData>
  implements IDeletable
{
  ...
  async listPublishedBlogs(
    ...
    const {
      ...
      nextCursor,
      hasMore,
    } = await this.paginate(q, { limit, cursor });
  ...
  private async _enrichBlogs(blogs: BlogData[]): Promise<BlogData[]> {
    return Promise.all(
      blogs.map(async (blog) => {
        const attachmentsQuery = buildAttachmentsQuery(
          this.attachmentsCol,
          blog.id,
        );
  ...
  async deleteBlog(id: string, authorId: string): Promise<void> {
    ...
    await Promise.all([
      ...attachments.map((a) =>
        this.storage.deleteObject(a?.['key'] as string),
      ),
  ...
  async deleteData(userId: string): Promise<void> {
    ...
    for (const doc of snapshot.docs) {
      await this.storage
        .deleteObject(`blogs/${doc.id}/content`)
        .catch(() => undefined);
  ...
  async unlikeBlog(userId: string, blogId: string): Promise<void> {
    ...
    const [blogDoc, likeDoc] = await Promise.all([
      blogRef.get(),
      likeRef.get(),
    ]);
