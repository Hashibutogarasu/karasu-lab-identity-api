...
export class InitializeService implements IBetterAuthBootStrapper {
  ...
  async bootstrap(): Promise<void> {
    ...
    const mailServiceInstance = mailService(emailConfig.RESEND_API_KEY, {
      name: emailConfig.EMAIL_FROM_NAME,
      address: emailConfig.EMAIL_FROM_ADDRESS,
    });
    this.context.dbService = new PostgresDatabaseService(
      authConfig.NODE_ENV,
      authConfig.DATABASE_URL,
    );
    this.context.notificationService = authNotificationServiceFactory(
      mailServiceInstance,
      configService,
    );
    ...
    this.context.auth = this.authFactory(
      ...
      this.context.rateLimitConfigInstance,
    );
  }
}
