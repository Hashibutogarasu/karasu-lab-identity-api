...
function createErrorMap<
  T extends Record<string, Record<string, readonly [string, ErrorStatus]>>,
>(
  map: T,
): {
  [K in keyof T]: {
...
export const ErrorCodes = createErrorMap({
  ...
  SYSTEM: {
    RESEND_API_KEY_REQUIRED: [
      'system.resend_api_key_required',
      'INTERNAL_SERVER_ERROR',
    ],
    AUTH_INITIALIZATION_FAILED: [
      'system.auth_initialization_failed',
      'INTERNAL_SERVER_ERROR',
    ],
    OPENAPI_UNINITIALIZED: [
      'system.openapi_uninitialized',
      'INTERNAL_SERVER_ERROR',
    ],
