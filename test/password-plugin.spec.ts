 ...
 */
async function createVerifiedUserAndSignIn(email: string, password: string): Promise<string> {
	...
	if (!cookie) {
		throw new Error(`sign-in returned no set-cookie (${signInRes.status}): ${await signInRes.text()}`);
	}
...
async function createSocialOnlyUserWithSession(email: string): Promise<string> {
	...
	const accounts: Array<{ id: string; providerId: string }> = await ctx.adapter.findMany({
		model: 'account',
...
describe('Password Plugin Integration Tests', () => {
	...
	it('A user without a password receives a different error than a credential user', async () => {
		...
		const credCookie = await createVerifiedUserAndSignIn(credEmail, credPassword);
